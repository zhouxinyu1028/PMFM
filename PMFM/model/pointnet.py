...
class STN3d(nn.Module):
    def __init__(self, isGroupNorm=False, num_gropus=32):
        ...
        # bn
        if self.isGroupNorm:
            self.bn1 = nn.GroupNorm(self.num_gropus, 64)
            self.bn2 = nn.GroupNorm(self.num_gropus, 128)
            self.bn3 = nn.GroupNorm(self.num_gropus, 1024)
            self.bn4 = nn.GroupNorm(self.num_gropus, 512)
            self.bn5 = nn.GroupNorm(self.num_gropus, 256)
        else:
...
class STNkd(nn.Module):
    def __init__(self, k=64, isGroupNorm=False, num_gropus=32):
...
class PointNetfeat(nn.Module):
    def __init__(self, global_feat=True, feature_transform=False, isGroupNorm=False, num_gropus=32):
        super(PointNetfeat, self).__init__()
        self.isGroupNorm = isGroupNorm
        self.num_gropus = num_gropus
        self.stn = STN3d(isGroupNorm=self.isGroupNorm, num_gropus=self.num_gropus)
        self.conv1 = torch.nn.Conv1d(3, 64, 1)
        self.conv2 = torch.nn.Conv1d(64, 128, 1)
        self.conv3 = torch.nn.Conv1d(128, 1024, 1)
        if isGroupNorm:
        ...
        if self.feature_transform:
            self.fstn = STNkd(k=64, isGroupNorm=self.isGroupNorm, num_gropus=self.num_gropus)
...
class PointNetDenseCls(nn.Module):
    def __init__(self, k=2, feature_transform=False, isGroupNorm=False, num_gropus=32):
        super(PointNetDenseCls, self).__init__()
        self.k = k
        self.isGroupNorm = isGroupNorm
        self.num_gropus = num_gropus
        self.feature_transform = feature_transform
        self.feat = PointNetfeat(global_feat=False, feature_transform=feature_transform, isGroupNorm=self.isGroupNorm, num_gropus=self.num_gropus)
        self.conv1 = torch.nn.Conv1d(1088, 512, 1)
        self.conv2 = torch.nn.Conv1d(512, 256, 1)
        self.conv3 = torch.nn.Conv1d(256, 128, 1)
        self.conv4 = torch.nn.Conv1d(128, self.k, 1)
        if self.isGroupNorm:
            self.bn1 = nn.GroupNorm(self.num_gropus, 512)
            self.bn2 = nn.GroupNorm(self.num_gropus, 256)
            self.bn3 = nn.GroupNorm(self.num_gropus, 128)
        else:
            self.bn1 = nn.BatchNorm1d(512)
            self.bn2 = nn.BatchNorm1d(256)

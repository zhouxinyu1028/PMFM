...
# 版本二
class PMM(nn.Module):
    """Point-cloud Matrix Module (PMM) based on PointNet."""

    def __init__(self, num_classes: int = 30, isGroupNorm=False, num_gropus=32):
        super(PMM, self).__init__()
        self.pointcls = PointNetDenseCls(k=num_classes, isGroupNorm=isGroupNorm, num_gropus=num_gropus)

    def forward(self, points):
        return self.pointcls(points)


class CFM(nn.Module):
    """Cross Fusion Module (CFM): PEM -> PMM probability matrix -> CT/MR fusion."""
    ...
    def __init__(self, num_classes: int = 30, isGroupNorm=False, num_gropus=32):
        ...
        self.pmm = PMM(num_classes=self.pointNet_num_classes, isGroupNorm=self.isGroupNorm,
    ...
    def deal_point(self, ct_pred, mr_pred):
        ...
        # TensorToPng(ct_pred.detach(), r"E:\datesets\HaNSeg\SI\gan_image\test\a", "test")
        # TensorToPng(mr_pred.detach(), r"E:\datesets\HaNSeg\SI\gan_image\test\b", "test")
        ...
        points, combined_orig_2d_points, index_C = self.pem.focus_filter(ct_pred[0], mr_pred[0])
        if len(points) == 0:
        ...
        # ------------------中心位置突出--------------------
        # 对齐每个类别
        numpy_combined = points
        # --------------------------------------------------
        combined_tensor = torch.from_numpy(numpy_combined)
        combined_tensor = combined_tensor.unsqueeze(dim=0).permute(0, 2, 1).to(ct_pred.device)
        combined_tensor = combined_tensor.to(dtype=ct_pred.dtype)

        result, _, _ = self.pmm(combined_tensor)  # (1, point_number, 30)
